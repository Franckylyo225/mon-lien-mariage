...
function SignupPage() {
  ...
  return (
    <AuthLayout
      ...
      title={<>Commençons <em className="text-[#E82050]">votre histoire.</em></>}
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        ...
        <label className="flex items-start gap-2 text-xs text-[#5A4F52]">
          <input
            type="checkbox"
            checked={cgu}
            onChange={(e) => setCgu(e.target.checked)}
            className="mt-0.5 accent-[#E82050]"
          />
          J'accepte les conditions générales d'utilisation.
        </label>
        {error ? <p className="text-xs text-destructive">{error}</p> : null}
        {info ? <p className="rounded-md border border-[#E82050]/30 bg-[#E82050]/10 px-3 py-2 text-xs text-[#C81A45]">{info}</p> : null}
        <button
          type="submit"
          disabled={loading}
...
export const inputClass =
  "w-full rounded-xl border border-[#F1E3C6]/70 bg-white/70 px-4 py-3 text-sm text-[#201A1C] outline-none transition placeholder:text-[#7A6D70] focus:border-[#E82050] focus:bg-white focus:ring-2 focus:ring-[#E82050]/20";
...
export function AuthLayout({
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#FBF8F8] text-[#201A1C]">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(900px 500px at 15% -10%, #FDF0F3 0%, #FBF8F8 55%, #FBF8F8 100%)",
        }}
      />
      <div aria-hidden className="pointer-events-none absolute inset-0 opacity-60">
        <span className="absolute left-[8%] top-[22%] size-2 rounded-full bg-[#E82050]/40" />
        <span className="absolute right-[42%] top-[8%] size-3 rounded-full bg-[#F1E3C6]/60" />
        <span className="absolute left-[38%] bottom-[10%] size-1.5 rounded-full bg-[#C6A15B]/50" />
      </div>
      ...
      <div className="relative mx-auto grid min-h-screen w-full max-w-6xl grid-cols-1 lg:grid-cols-2">
        ...
        <div className="flex flex-col px-5 py-8 sm:px-10 sm:py-12">
          <Link to="/" className="inline-block font-serif text-lg italic">
            MonInvit<span className="text-[#E82050]">.com</span>
          </Link>
          ...
          <p className="mt-10 text-center text-[11px] text-[#7A6D70] sm:text-left">
            © 2026 MonInvit.com
          </p>
        </div>
        ...
        <aside className="relative hidden overflow-hidden lg:block">
          <img
            ...
            alt=""
            aria-hidden
            className="absolute inset-4 h-[calc(100%-2rem)] w-[calc(100%-2rem)] rounded-[2rem] object-cover shadow-2xl shadow-[#E82050]/20"
          />
          <div className="absolute inset-4 rounded-[2rem] bg-gradient-to-t from-[#201A1C]/70 via-transparent to-transparent" />
          <div className="absolute bottom-14 left-12 right-12 text-[#FBF8F8]">
            <p className="font-serif text-3xl italic leading-[1.15]">
              « Notre page était prête <br />
              le soir même. »
            </p>
            <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.35em] text-[#F1E3C6]">
              Aïcha & Loïc — Abidjan
            </p>
          </div>
        </aside>
