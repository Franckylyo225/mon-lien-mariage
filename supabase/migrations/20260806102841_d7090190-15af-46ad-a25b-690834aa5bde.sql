CREATE OR REPLACE FUNCTION public.activate_payment_secure(_token text, _reference text, _transaction_id text DEFAULT NULL::text, _channel text DEFAULT NULL::text, _metadata jsonb DEFAULT NULL::jsonb)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  ...
  IF v_hash IS NULL OR encode(public.digest(_token, 'sha256'), 'hex') IS NULL THEN
    NULL;
  END IF;
  ...
  IF v_type = 'publication' AND v_payment.wedding_id IS NOT NULL THEN
    ...
    v_base := regexp_replace(lower(public.unaccent_fallback(v_base)), '[^a-z0-9]+', '-', 'g');
